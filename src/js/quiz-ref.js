export const ref = {
  progressStart: document.querySelector('.progress__section'),
  backButton: document.querySelector('.progress__button-link--back'),
  cardNextButton: document.querySelector('.card__button-link'),
  answerList: document.querySelector('.quiz-list'),
  answerListItem: document.querySelector('.quiz-list__item'),
  question: document.querySelector('.hero__heading'),
  heading: document.querySelector('.hero__heading-wrap'),
  headerLogo: document.querySelector('.heading__logo'),
  cardWrap: document.querySelector('.card-wrap'),
  containerHero: document.querySelector('.hero__container'),
  backgroundMask: document.querySelector('.hero__mask-svg'),
  cardWrapContent: document.querySelector('.card-wrap__content'),
  cardButtonWrap: document.querySelector('.card__button-wrap '),
  choiceForm: document.querySelector('.quiz__choice-form'),
  choiceFormDesc: document.querySelector('.quiz__choice-form-desc'),
  choiceFormWrap: document.querySelector('.quiz__choice-form-wrap'),
  choiceFormChechboxAll: document.querySelectorAll('.quiz__choice-hidden-checkbox'),
  choiceButton: document.querySelector('.quiz__choice__button-link'),
  choiceButtonWrap: document.querySelector('.quiz__choice__button-wrap'),
  choiceFormLine: document.querySelector('.quiz__choice-line-form'),
  choiceFormLineDesc: document.querySelector('.quiz__choice-line-form-desc'),
  choiceFormLineWrap: document.querySelector('.quiz__choice-line-form-wrap'),
  choiceLineButtonWrap: document.querySelector('.quiz__choice-line__button-wrap'),
  choiceLineButton: document.querySelector('.quiz__choice-line__button-link'),
  dotParent: document.querySelector('.progress-bar__dot'),
  allDot: document.querySelector('.progress-bar__dot').children,
  line: document.querySelector('.progress-bar__line-done'),
  lineWrap: document.querySelector('.progress-bar__line'),
  percentage: document.querySelector('.progress__percentage-value'),
  payFormLabel: document.querySelector('.pay__payment-icon-wrap'),
  payFormIconWrap: document.querySelector('.pay__payment-icon-wrap'),
};
